...
The installed Silero TorchScript model evaluates one 32 ms frame per call. This
...
def _installed_model_path(expected_version: str) -> Path:
    try:
...
def _load_sequence_model(sampling_rate: int, expected_version: str) -> _SequenceModel:
    ...
    # Loading the data file directly avoids importing silero_vad.model, which
    # changes PyTorch's process-wide intra-op thread count.
    source = torch.jit.load(
        str(_installed_model_path(expected_version)), map_location="cpu"
    ).eval()
